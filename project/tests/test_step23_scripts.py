...
def test_run_step23_protocol_and_analyze_interface_topology(tmp_path: Path):
    outdir = tmp_path / "step23_protocol"
    env = os.environ.copy()
    ...
    run_cmd = [
        sys.executable,
        str(ROOT / "scripts" / "run_step23_protocol.py"),
        "--num-cells",
        "120",
        "--t-end",
        "0.18",
    ...
    analyze_cmd = [
        sys.executable,
        str(ROOT / "scripts" / "analyze_interface_topology.py"),
        "--input",
        str(outdir / "translation_x_pos" / "interface_topology_trace.json"),
        "--title",
        "Step 23 topology analysis test",
        "--output",
        str(outdir / "analysis_translation_x_pos"),
    ]
    subprocess.run(analyze_cmd, check=True, cwd=ROOT, env=env)

    analysis_path = outdir / "analysis_translation_x_pos" / "interface_topology_analysis.json"
    assert analysis_path.exists()
    analysis = json.loads(analysis_path.read_text(encoding="utf-8"))
    for track_name, payload in analysis["external_interpretation"].items():
        assert payload["axial_minus_swirl_topology"] > 0.0
