...
# carried forward unchanged from v32 standard seed-expansion sign-aware fallback candidate
...
def main() -> None:
    p = argparse.ArgumentParser(description='Stress-test the carried-forward v32 sign-aware gate fallback on N224 harder unseen nuisance without changing the overview-first mainline or the farther-scale separator.')
    p.add_argument('--panel-root', type=str, default='/mnt/data/v33_harder_raw')
    p.add_argument('--outdir', type=str, default='outputs/stage1_global_overview_farther_scale_n224_harder_nuisance_sign_aware_gate_fallback_candidate')
    ...
    payload = {
        'protocol': 'stage1_global_overview_farther_scale_n224_harder_nuisance_sign_aware_gate_fallback_candidate',
        'panel_root': args.panel_root,
        'selection_rule': 'carry forward the v32 sign-aware fallback unchanged into N224 harder unseen nuisance: keep the overview-first mainline unchanged, keep the farther-scale bidirectional rotation separator unchanged, and keep the v32 sign-aware fallback thresholds fixed; no threshold reselection and no target-scale tuning',
        ...
        'verdict': 'harder_nuisance_stress_test',
        'interpretation': 'This audit asks whether the exact v32 sign-aware fallback carries cleanly into the N224 harder unseen-nuisance panel without changing thresholds or mainline structure.',
    }
    (outdir / 'stage1_global_overview_farther_scale_n224_harder_nuisance_sign_aware_gate_fallback_candidate_analysis.json').write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    remaining = [p for p in target_eval['predictions'] if p['predicted'] != p['label']]
    lines = [
        '# Stage-1 farther-scale N224 harder unseen-nuisance sign-aware gate-fallback candidate',
        '',
        'This stress test keeps the **v32 sign-aware fallback unchanged** and applies it to the harder unseen-nuisance N224 panel (`early_soft / mid_sharp / late_balanced`).',
        '',
        ...
        f'- `neg_over_pos_sign_ratio` must be `<= {FIXED_NEG_OVER_POS_RATIO_THRESHOLD:.6f}`',
        f'- `translation_distance - nontranslation_distance` must be `<= {FIXED_GATE_MARGIN_BRIDGE_THRESHOLD:.6f}`',
    ...
    if remaining:
        ...
        lines.append('- none on the current N224 harder unseen-nuisance panel')
    lines.extend([
        '',
        '## Interpretation',
        '- This is a stricter test than v32 because the harder unseen-nuisance panel changes the profile family while keeping the fallback fixed.',
        '- If the fallback does not trigger spuriously and does not introduce new errors, that is evidence that it is behaving like a controlled low-energy negative-translation recovery rule rather than a panel-specific patch.',
        '- If it fails or misfires, the next branch should move toward richer temporal / trace-aware gate basis work rather than adding another narrow fallback.',
        '',
        '## Mainline impact',
        '- keep the overview-first mainline unchanged',
        '- keep the farther-scale bidirectional rotation separator unchanged',
        '- evaluate whether the sign-aware fallback remains a promotable controlled fallback after this harder-nuisance stress test',
    ])
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FARTHER_SCALE_N224_HARDER_NUISANCE_SIGN_AWARE_GATE_FALLBACK_CANDIDATE_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
