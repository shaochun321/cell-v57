...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--floor-json', default=str(ROOT / 'outputs/process_summary_translation_polarity_resolution_floor_audit_r1/process_summary_translation_polarity_resolution_floor_audit.json'))
...
def run_protocol(args: argparse.Namespace) -> dict[str, str | dict]:
    ...
    payload = audit_translation_x_neg_sign_limited_polarity_basis_files(
        carrier_path=args.carrier_json,
        floor_path=args.floor_json,
        decomposition_path=args.decomposition_json,
    )
    out_path = outdir / 'process_summary_translation_x_neg_sign_limited_polarity_basis_audit.json'
