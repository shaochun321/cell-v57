...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run the Step 24 interface-temporal protocol.")
    p.add_argument("--num-cells", type=int, default=180)
    p.add_argument("--t-end", type=float, default=0.18)
    p.add_argument("--dt", type=float, default=0.001)
    p.add_argument("--outdir", type=str, default="outputs/step24_protocol")
...
def _extract_report(summary: dict) -> dict:
    diag = summary.get("interface_temporal_diagnostics", {})
    tracks = diag.get("tracks", {})
    out: dict[str, dict] = {}
    for name, payload in tracks.items():
        out[name] = {
            "active_family_attenuation": {
                fam: payload.get("active_families", {}).get(fam, {}).get("mean_attenuation_index", 0.0)
                for fam in payload.get("active_families", {}).keys()
            },
            "active_family_outer_inner_ratio": {
                fam: payload.get("active_families", {}).get(fam, {}).get("mean_outer_inner_ratio", 0.0)
                for fam in payload.get("active_families", {}).keys()
            },
            "active_family_peak_times": payload.get("active_peak_times", {}),
            "active_axis_polarity_balance": payload.get("active_mean_axis_polarity_balance", {}),
            "active_mean_signed_circulation": payload.get("active_mean_signed_circulation", 0.0),
            "active_transfer_attenuation": payload.get("active_transfer", {}).get("mean_attenuation_index", 0.0),
            "active_bandwidth_attenuation": payload.get("active_bandwidth", {}).get("mean_attenuation_index", 0.0),
        }
    return out
...
def _run_one(base_outdir: Path, name: str, **kwargs) -> dict:
    ...
    return {
        ...
        "temporal_report": _extract_report(result.summary),
    }
