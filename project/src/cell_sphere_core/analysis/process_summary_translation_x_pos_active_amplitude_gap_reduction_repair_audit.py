...
def build_translation_x_pos_active_amplitude_gap_reduction_repair_audit(
    *,
    ...
    evidence = {
        'seed8_translation_x_pos_gap_reduced_vs_round37': current_gap < previous_gap,
        'seed8_translation_x_pos_active_amplitude_improved_vs_round37': txp8['active_mean_polarity_projection'] > float(previous_round.get('seed8', {}).get('translation_x_pos', {}).get('active_mean_polarity_projection', 0.0)),
        ...
        'seed8_translation_x_pos_carrier_summary_exceeds_raw_mean': abs(txp8['active_carrier_mean_polarity_projection']) >= abs(txp8['active_raw_mean_polarity_projection']),
    }
    contracts_passed = all(evidence.values())
    residual_issue = 'seed8_translation_x_pos_gap_to_seed7_still_visible' if current_gap > 0.12 else 'none'

    return {
        'suite': 'translation_x_pos_active_amplitude_gap_reduction_repair_audit_r1',
        ...
        'seed8': seed8,
        'evidence': evidence,
        'inferred_outcome': 'translation_x_pos_active_amplitude_gap_reduction_repair_success' if contracts_passed else 'undetermined',
        'residual_issue': residual_issue,
        'interpretation': {
            'primary_effect': 'reduce the residual seed8-to-seed7 x_pos active amplitude gap by trusting translation-carrying x pairs more strongly when the raw mean is still diluted by weak static x pairs',
            'guardrail': 'preserve x-axis identity, polarity sign, strongest-pair translation mode, and the rotation_z_pos z-axis guardrail',
            'next_branch': 'optional only if future rounds need seed8 x_pos amplitude to move materially closer to the seed7 carrier without broadening the repair scope',
        },
    }
