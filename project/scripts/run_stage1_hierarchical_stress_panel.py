...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run a richer perturbation stress panel for the stage-1 hierarchical readout.')
    p.add_argument('--outdir', type=str, default='outputs/stage1_hierarchical_stress_panel')
...
def build_cases() -> dict[str, dict]:
    return {
        ...
        'translation_x_pos_early_soft': {
            ...
            'vestibular_onset_fraction': 0.30,
            'vestibular_duration_fraction': 0.65,
        },
        'translation_x_pos_late_sharp': {
            'disable_gravity': True,
            'vestibular_motion': 'translation',
            'vestibular_linear_axis': 'x',
            'vestibular_linear_sign': 1.0,
            ...
            'vestibular_duration_fraction': 0.28,
        },
...
def main() -> None:
    ...
    manifest: dict[str, object] = {
        'protocol': 'stage1_hierarchical_stress_panel',
    ...
    (outdir / 'stage1_hierarchical_stress_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2))
    print(f'[OK] stress panel written to {outdir}')
