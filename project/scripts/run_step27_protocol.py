...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run the Step 27 cross-protocol/cross-parameter motif stability sweep.")
    ...
    p.add_argument("--outdir", type=str, default="outputs/step27_protocol_sample")
    return p.parse_args()
...
def main() -> None:
    ...
    soft_cells = max(90, args.num_cells - 20)
    ...
    report_path = outdir / "step27_protocol_report.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    invariants = summarize_channel_invariants(report)
    invariants_path = outdir / "step27_invariants.json"
    invariants_path.write_text(json.dumps(invariants, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"输出目录: {outdir}")
    print(f"协议报告: {report_path}")
    print(f"稳定性/不变量: {invariants_path}")
