...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--anatomy-json', default=str(ROOT / 'outputs/process_summary_translation_discrete_channel_anatomy_audit_r1/process_summary_translation_discrete_channel_anatomy_audit.json'))
...
def run_protocol(args: argparse.Namespace) -> dict[str, str | dict]:
    ...
    payload = audit_translation_carrier_smoothing_sensitivity_files(anatomy_path=args.anatomy_json, carrier_path=args.carrier_json)
    out_path = outdir / 'process_summary_translation_carrier_smoothing_sensitivity_audit.json'
