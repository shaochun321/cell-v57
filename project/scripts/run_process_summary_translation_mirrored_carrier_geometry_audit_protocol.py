...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--seed-profiles', default=str(ROOT / 'outputs/process_summary_translation_mirrored_readout_audit_r1/process_summary_translation_mirrored_readout_seed_profiles.json'))
...
def run_protocol(args: argparse.Namespace) -> dict[str, str | dict]:
    ...
    payload = audit_translation_mirrored_carrier_geometry_files(seed_profiles_path=args.seed_profiles, decomposition_path=args.decomposition_json)
    out_path = outdir / 'process_summary_translation_mirrored_carrier_geometry_audit.json'
