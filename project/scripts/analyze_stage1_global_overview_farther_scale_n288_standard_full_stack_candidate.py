...
def main() -> None:
    ...
    p.add_argument('--panel-root', type=str, default='/mnt/data/v40_n288_standard_raw')
    p.add_argument('--outdir', type=str, default='outputs/stage1_global_overview_farther_scale_n288_standard_full_stack_candidate')
    ...
    payload = {
        'protocol': 'stage1_global_overview_farther_scale_n288_standard_full_stack_candidate',
        ...
        'interpretation': 'This first beyond-N224 probe keeps the current full rule stack fixed and asks whether the overview-first geometry, the bidirectional separator, the negative sign-aware fallback, and the positive trace-basis bridge remain controlled at N288 under the standard richer-profile panel.'
    }
    (outdir / 'stage1_global_overview_farther_scale_n288_standard_full_stack_candidate_analysis.json').write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    lines = [
        '# Stage-1 farther-scale N288 standard full-stack candidate',
        '',
        'This probe carries forward the **current fixed rule stack** into the first beyond-N224 standard richer-profile panel.',
        '',
        '## Fixed stack',
    ...
    lines += [
        '',
        '## Interpretation',
        '- this is the first beyond-N224 pressure test of the full current rule stack',
        ...
        '- if N288 standard is clean, the next honest move is N288 harder unseen-nuisance before broader promotion',
    ]
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FARTHER_SCALE_N288_STANDARD_FULL_STACK_CANDIDATE_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
