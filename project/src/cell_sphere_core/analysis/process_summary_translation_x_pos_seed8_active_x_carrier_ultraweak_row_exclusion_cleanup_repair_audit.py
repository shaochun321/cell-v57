...
def build_translation_x_pos_seed8_active_x_carrier_ultraweak_row_exclusion_cleanup_repair_audit(
    *,
    ...
    evidence = {
        "seed8_translation_x_pos_gap_reduced_vs_round43": current_gap < previous_gap,
        "seed8_translation_x_pos_active_amplitude_improved_vs_round43": txp8["active_mean_polarity_projection"] > float(previous_round.get("seed8", {}).get("translation_x_pos", {}).get("active_mean_polarity_projection", 0.0)),
    ...
    return {
        "suite": "translation_x_pos_seed8_active_x_carrier_ultraweak_row_exclusion_cleanup_repair_audit_r1",
        ...
        "seed8": seed8,
        "evidence": evidence,
        "inferred_outcome": "translation_x_pos_seed8_active_x_carrier_ultraweak_row_exclusion_cleanup_repair_success" if contracts_passed else "undetermined",
        "residual_issue": residual_issue,
        "interpretation": {
            "primary_effect": "raise the repaired seed8 translation_x_pos mean slightly further by excluding ultraweak active x rows only when strongest-pair translation mode is already restored, the carrier-floor weighted signal still has the same sign, and the raw mean remains clearly diluted below that floor-cleared carrier signal",
            ...
            "next_branch": "optional only if future rounds still need more seed8 x_pos lift after ultraweak-row exclusion without broadening beyond active x-family summary corrections",
        },
    }
