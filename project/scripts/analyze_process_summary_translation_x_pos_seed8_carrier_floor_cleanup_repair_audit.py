...
def main() -> None:
    ap = argparse.ArgumentParser(description="Analyze translation_x_pos seed8 carrier floor cleanup repair audit.")
    ...
    (outdir / "process_summary_translation_x_pos_seed8_carrier_floor_cleanup_repair_audit_analysis.json").write_text(
        json.dumps(analysis, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

    labels = ["seed7 x_pos", "seed8 x_pos", "seed8 x_pos raw", "seed8 x_pos carrier", "seed8 x_pos carrier-floor"]
    values = [
    ...
    ax.set_title("Round40 residual seed8 carrier-floor cleanup")
    ax.axhline(0.03, linestyle="--", linewidth=1.0)
    fig.tight_layout()
    fig.savefig(outdir / "process_summary_translation_x_pos_seed8_carrier_floor_cleanup_repair_audit.png", dpi=160)
