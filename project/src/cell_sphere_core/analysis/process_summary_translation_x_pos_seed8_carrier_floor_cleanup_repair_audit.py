...
def build_translation_x_pos_seed8_carrier_floor_cleanup_repair_audit(
    *,
    ...
    evidence = {
        "seed8_translation_x_pos_gap_reduced_vs_round39": current_gap < previous_gap,
        "seed8_translation_x_pos_active_amplitude_improved_vs_round39": txp8["active_mean_polarity_projection"] > float(previous_round.get("seed8", {}).get("translation_x_pos", {}).get("active_mean_polarity_projection", 0.0)),
        ...
        "seed8_translation_x_pos_carrier_floor_exceeds_current_mean": abs(txp8["active_carrier_floor_mean_polarity_projection"]) >= abs(txp8["active_mean_polarity_projection"]),
    ...
    return {
        "suite": "translation_x_pos_seed8_carrier_floor_cleanup_repair_audit_r1",
        ...
        "seed8": seed8,
        "evidence": evidence,
        "inferred_outcome": "translation_x_pos_seed8_carrier_floor_cleanup_repair_success" if contracts_passed else "undetermined",
        "residual_issue": residual_issue,
        "interpretation": {
            "primary_effect": "clean up the residual seed8 translation_x_pos carrier floor by trusting only positive-margin active x carriers that also clear a minimal strength or gate floor, instead of letting weak static x pairs keep dragging the repaired mean shallow",
            ...
            "next_branch": "optional only if future rounds still need more seed8 x_pos lift after carrier-floor cleanup without broadening beyond active x-family summary corrections",
        },
    }
