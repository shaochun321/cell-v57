...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run Phase R.5 narrow repeatability audit for rotation stability.')
    ...
    p.add_argument('--rotation-alphas', type=str, default='280,300,320,340,360,380')
    p.add_argument('--swirl-gains', type=str, default='1.00,1.05,1.10,1.15,1.20,1.25')
    p.add_argument('--seeds', type=str, default='7,8,9')
    p.add_argument('--outdir', type=str, default='outputs/phase_r5_sample')
    return p.parse_args()
...
def main() -> None:
    ...
    central_alpha = alphas[min(len(alphas)//2, len(alphas)-1)] if alphas else 320.0
    central_gain = gains[min(len(gains)//2, len(gains)-1)] if gains else 1.10
    ...
    for alpha in alphas:
        for gain in gains:
            ...
            for rep_idx, seed in enumerate(seeds, start=1):
                rep_name = f'{case_id}_s{seed}'
                repeats.append({
                    'repeat_id': f'r{rep_idx}',
                    'rng_seed': seed,
                    'rotation_pos': _run_one(
                        outdir, rep_name + '_pos',
                        num_cells=args.num_cells, t_end=args.t_end, dt=args.dt,
    ...
    report_path = outdir / 'phase_r5_protocol_report.json'
    ...
    print(f'Phase R.5 审计: {audit_path}')
