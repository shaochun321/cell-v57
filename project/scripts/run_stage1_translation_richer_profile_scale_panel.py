...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run translation-only richer-profile multi-scale panel.')
    p.add_argument('--outdir', type=str, default='outputs/stage1_translation_richer_profile_scale_panel_raw')
...
def semantic_label(case_name: str) -> str:
    return 'translation_x_pos' if 'x_pos' in case_name else 'translation_x_neg'


def profile_label(case_name: str) -> str:
    if 'early_sharp' in case_name:
        return 'early_sharp'
    if 'mid_balanced' in case_name:
        return 'mid_balanced'
    return 'late_soft'
...
def main() -> None:
    ...
    manifest: dict[str, object] = {
        'protocol': 'stage1_translation_richer_profile_scale_panel',
            ...
            for case_name, overrides in cases.items():
                ...
                try:
                    ...
                    manifest['cases'][f'N{num_cells}/seed_{seed}/{case_name}'] = {
                        ...
                        'profile': profile_label(case_name),
                        'config': asdict(cfg),
                        'summary': summary,
                    }
    ...
    (outdir / 'stage1_translation_richer_profile_scale_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2))
    print(f'[OK] translation richer profile scale panel written to {outdir}')
