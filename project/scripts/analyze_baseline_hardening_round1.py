...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Analyze baseline hardening round 1 results.')
    p.add_argument('--input', required=True, type=str)
    p.add_argument('--title', type=str, default='Baseline Hardening Round 1')
...
def build_analysis(report: dict) -> dict:
    ...
    analysis = {
        ...
        'static': {
            ...
            'tail_volume_ratio_mean': float(static_case.get('equilibrium_diagnostics', {}).get('tail_volume_ratio_mean', 0.0)),
            'tail_shape_mean': float(static_case.get('equilibrium_diagnostics', {}).get('tail_shape_mean', 0.0)),
            'quietness_score': float(1.0 / (1.0 + static_case.get('equilibrium_diagnostics', {}).get('tail_kinetic_mean', 0.0))),
            'shape_score': float(static_case.get('equilibrium_diagnostics', {}).get('tail_volume_ratio_mean', 0.0) / max(1.0 + static_case.get('equilibrium_diagnostics', {}).get('tail_shape_mean', 0.0), 1e-9)),
        },
        'translation': {'tracks': {}},
        'rotation': {'tracks': {}},
    }
...
def render_png(analysis: dict, output_dir: Path, title: str) -> None:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / 'baseline_hardening_round1_overview.png', dpi=180)
    plt.close()
...
def main() -> None:
    ...
    (output_dir / 'baseline_hardening_round1_analysis.json').write_text(json.dumps(analysis, ensure_ascii=False, indent=2), encoding='utf-8')
