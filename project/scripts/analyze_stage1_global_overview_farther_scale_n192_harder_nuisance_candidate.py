...
def main() -> None:
    p = argparse.ArgumentParser(description='Evaluate the current farther-scale N192 overview-first branch on a harder unseen-nuisance panel.')
    p.add_argument('--panel-root', type=str, default='outputs/stage1_global_overview_farther_scale_n192_harder_nuisance_panel_raw')
    p.add_argument('--outdir', type=str, default='outputs/stage1_global_overview_farther_scale_n192_harder_nuisance_candidate')
    ...
    payload = {
        'protocol': 'stage1_global_overview_farther_scale_n192_harder_nuisance_candidate',
        'panel_root': args.panel_root,
        'selection_rule': 'evaluate the current farther-scale N192 overview-first branch on unseen harder nuisance profiles without reselecting any keys or thresholds on target-scale data; the existing mid-balanced rotation veto is carried forward unchanged but only activates if its profile condition is encountered',
        ...
        'verdict': 'harder_farther_scale_validation',
        'interpretation': 'This audit tests whether the current farther-scale N192 branch remains stable on harder unseen nuisance profiles rather than only the standard richer-profile farther-scale panel.',
    }
    (outdir / 'stage1_global_overview_farther_scale_n192_harder_nuisance_candidate_analysis.json').write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    lines = [
        '# Stage-1 farther-scale N192 harder nuisance candidate',
        '',
        'This audit evaluates the current farther-scale N192 overview-first branch on a panel with **harder unseen nuisance profiles**:',
        '- `early_soft`',
        '- `mid_sharp`',
        '- `late_balanced`',
        '',
        ...
        '- no key reselection',
        '- no threshold retuning on N192 labels',
        '- the existing mid-balanced rotation veto is carried forward unchanged',
        '',
        '## Fixed keys',
        ...
        '',
        '## Interpretation',
        '- This is the next authoritative test after the standard richer-profile N192 veto candidate.',
        '- Because the harder nuisance panel does not contain `mid_balanced`, the carried-forward veto should not be needed unless the profile set is expanded later.',
        '- This therefore checks whether the farther-scale branch survives a harder OOD panel without leaning on the mid-balanced veto path.',
        '',
    ...
    if seen_errs:
        ...
        lines.append('- none')
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FARTHER_SCALE_N192_HARDER_NUISANCE_CANDIDATE_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print(f'[OK] wrote harder farther-scale audit to {outdir}')
