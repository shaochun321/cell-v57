...
def eval_dataset(dataset: list[dict[str, Any]], t_center, nt_center, b_center, r_center, pos_center, neg_center) -> tuple[dict[str, Any], dict[str, list[dict[str, Any]]]]:
...
def main() -> None:
    ap = argparse.ArgumentParser(description='Evaluate the fixed three-guard stack on N288 harder target-seed expansion.')
    ...
    ap.add_argument('--outdir', default='/mnt/data/v51_n288_harder_target_seedexp_fixed_three_guard_candidate')
    args = ap.parse_args()
    ...
    payload = {
        'protocol': 'stage1_global_overview_farther_scale_n288_harder_target_seedexp_fixed_three_guard_candidate',
        'seen_root': args.seen_root,
        'target_root': args.target_root,
        'selection_rule': 'carry forward the fixed three-guard stack unchanged into N288 harder target-seed expansion: keep the overview-first mainline unchanged, keep the fixed bidirectional separator unchanged, keep the legacy negative sign-aware fallback unchanged, keep the legacy positive trace-basis bridge unchanged, keep the v47 negative translation-margin guard unchanged, keep the v48 baseline-support guard unchanged, and keep the v50 positive translation-margin guard unchanged; no threshold reselection and no target-scale tuning',
        ...
        'verdict': 'n288_harder_target_seedexp_probe',
        'interpretation': 'This target-seed-expansion evaluation keeps the current fixed three-guard stack unchanged and asks whether the overview-first geometry plus the carried-forward controlled guards remain sparse and clean when the harder N288 target panel is broadened from seeds 7-8 to seeds 7-12.',
    }

    json_path = outdir / 'stage1_global_overview_farther_scale_n288_harder_target_seedexp_fixed_three_guard_candidate_analysis.json'
    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    target_errors = payload['target_misclassifications']
    lines = [
        '# Stage-1 farther-scale N288 harder target-seed expansion fixed three-guard candidate',
        '',
        'This audit carries forward the **fixed three-guard stack** into the harder N288 target panel with target seeds expanded from `7-8` to `7-12`.',
        '',
        '## Fixed stack',
        '- overview-first mainline unchanged',
        f'- bidirectional rotation separator: `{BIDIRECTIONAL_VETO_KEY} > {FIXED_SEPARATOR_THRESHOLD:.6f}`',
        f'- v47 negative translation-margin guard: `gate_margin_translation_minus_nontranslation <= {TRANSLATION_MARGIN_GUARD_THRESHOLD:.6f}` when pre-veto is `translation_x_neg`',
        f'- v48 baseline-support guard: `{POSITIVE_TRACE_FEATURE} <= {BASELINE_SUPPORT_THRESHOLD:.6f}` when current prediction is `translation_x_pos`',
        f'- v50 positive translation-margin guard: `gate_margin_translation_minus_nontranslation <= {POSITIVE_TRANSLATION_MARGIN_GUARD_THRESHOLD:.6f}` when pre-veto is `translation_x_pos`',
        f'- legacy positive bridge retained on `{POSITIVE_TRACE_FEATURE}` ({POSITIVE_TRACE_DIRECTION} / `{POSITIVE_TRACE_THRESHOLD:.6f}`)',
        '',
        '## Results',
    ...
    lines += [
        '',
        '## Interpretation',
        '- this is not a same-panel recheck; it is the first harder N288 target-seed broadening under the fixed three-guard stack',
        '- the only honest question is whether new farther-scale seed families appear without retuning any guard',
        '- if new residuals remain, they should be treated as structured audit targets, not as excuses to silently accumulate rules',
    ]
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FARTHER_SCALE_N288_HARDER_TARGET_SEEDEXP_FIXED_THREE_GUARD_CANDIDATE_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
