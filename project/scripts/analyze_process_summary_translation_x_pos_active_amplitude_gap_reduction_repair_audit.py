...
def main() -> None:
    ap = argparse.ArgumentParser(description='Analyze translation_x_pos active amplitude gap reduction repair audit.')
    ...
    (outdir / 'process_summary_translation_x_pos_active_amplitude_gap_reduction_repair_audit_analysis.json').write_text(
        json.dumps(analysis, ensure_ascii=False, indent=2),
        encoding='utf-8',
    )

    labels = ['seed7 x_pos', 'seed8 x_pos', 'seed8 x_pos raw', 'seed8 x_pos weighted', 'seed8 x_pos carrier']
    values = [
        ...
        float(audit['seed8']['translation_x_pos']['active_support_weighted_mean_polarity_projection']),
    ...
    ax.set_title('Round38 residual seed8-to-seed7 x_pos amplitude gap reduction')
    ax.axhline(0.03, linestyle='--', linewidth=1.0)
    fig.tight_layout()
    fig.savefig(outdir / 'process_summary_translation_x_pos_active_amplitude_gap_reduction_repair_audit.png', dpi=160)
