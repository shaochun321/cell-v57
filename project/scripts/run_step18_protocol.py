...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run the Step 18 mirror-interface protocol.")
    p.add_argument("--num-cells", type=int, default=180)
    p.add_argument("--t-end", type=float, default=0.18)
    p.add_argument("--dt", type=float, default=0.001)
    p.add_argument("--outdir", type=str, default="outputs/step18_protocol")
...
def _run_one(base_outdir: Path, name: str, **kwargs) -> dict:
    ...
    interface_trace = json.loads((outdir / "interface_trace.json").read_text(encoding="utf-8"))
    return {
        "summary": result.summary,
        "mirror_interface_summary": result.summary.get("mirror_interface_diagnostics", {}),
        "readout_summary": result.summary.get("external_readout_diagnostics", {}),
        "interface_trace_samples": len(interface_trace),
    }
...
def main() -> None:
    ...
    report_path = outdir / "step18_protocol_report.json"
