...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run the Step 21 channel-family lineage protocol.")
    p.add_argument("--num-cells", type=int, default=180)
    p.add_argument("--t-end", type=float, default=0.18)
    p.add_argument("--dt", type=float, default=0.001)
    p.add_argument("--outdir", type=str, default="outputs/step21_protocol")
...
def _extract_lineage_report(summary: dict) -> dict:
    diag = summary.get("interface_lineage_diagnostics", {})
    tracks = diag.get("tracks", {})
    out: dict[str, dict] = {}
    for name, payload in tracks.items():
        out[name] = {
            "family_means": payload.get("family_means", {}),
            ...
            "family_axis_balance": payload.get("family_axis_balance", {}),
            "active_family_axis_balance": payload.get("active_family_axis_balance", {}),
...
def _run_one(base_outdir: Path, name: str, **kwargs) -> dict:
    ...
    return {
        ...
        "lineage_report": _extract_lineage_report(result.summary),
    }
