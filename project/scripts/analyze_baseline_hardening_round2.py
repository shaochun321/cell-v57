...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Analyze baseline hardening round 2 results.')
    p.add_argument('--input', required=True, type=str)
    p.add_argument('--title', type=str, default='Baseline Hardening Round 2')
...
def render_png(analysis: dict, output_dir: Path, title: str) -> None:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / 'baseline_hardening_round2_overview.png', dpi=180)
    plt.close()
...
def main() -> None:
    ...
    (output_dir / 'baseline_hardening_round2_analysis.json').write_text(json.dumps(analysis, ensure_ascii=False, indent=2), encoding='utf-8')
