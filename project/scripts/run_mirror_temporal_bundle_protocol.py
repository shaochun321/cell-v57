...
from cell_sphere_core.analysis.mirror_temporal_bundle import (
    build_mirror_temporal_bundle_trace_from_files,
    summarize_mirror_temporal_bundle_trace,
)
try:
    from scripts.run_mirror_channel_atlas_protocol import CASE_ORDER, run_protocol as run_atlas_protocol
except ModuleNotFoundError:  # pragma: no cover
    from run_mirror_channel_atlas_protocol import CASE_ORDER, run_protocol as run_atlas_protocol


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run the mirrored temporal bundle protocol on the current hardened baseline.')
    ...
    p.add_argument('--outdir', type=str, default='outputs/mirror_temporal_bundle_protocol_r1')
    p.add_argument('--reuse-existing', action='store_true', help='reuse existing mirror_channel_atlas traces under outdir instead of rerunning upstream protocol')
    return p.parse_args()
...
def run_protocol(args: argparse.Namespace) -> dict[str, Any]:
    ...
    atlas_report = None if reuse_existing else run_atlas_protocol(args)
    report = {
        'suite': 'mirror_temporal_bundle_protocol',
        'config': vars(args),
        'case_order': list(CASE_ORDER),
        'atlas_report_path': str(atlas_report['report_path']) if atlas_report is not None else '',
        'cases': {},
    }
    for case_name in CASE_ORDER:
        ...
        bundle_trace = build_mirror_temporal_bundle_trace_from_files(atlas_trace_path=atlas_trace_path)
        bundle_trace_path = case_dir / 'mirror_temporal_bundle_trace.json'
        bundle_trace_path.write_text(json.dumps(bundle_trace, ensure_ascii=False, indent=2), encoding='utf-8')
        bundle_summary = summarize_mirror_temporal_bundle_trace(bundle_trace)
        report['cases'][case_name] = {
            'mirror_channel_atlas_trace_file': atlas_trace_path.name,
            'mirror_temporal_bundle_trace_file': bundle_trace_path.name,
            'mirror_temporal_bundle_summary': bundle_summary,
        }
