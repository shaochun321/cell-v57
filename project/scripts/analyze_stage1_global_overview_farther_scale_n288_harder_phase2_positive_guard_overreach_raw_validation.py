...
POSITIVE_GUARD_RESTORE_CURL_FLOOR = 3.8
...
def eval_dataset(dataset: list[dict[str, Any]], t_center, nt_center, b_center, r_center, pos_center, neg_center):
    ...
    acts = {
        ...
        'positive_guard_overreach_restore_triggered': [],
    }
    for s in dataset:
        ...
        if dt < dnt:
            ...
            row['positive_guard_overreach_restore_triggered'] = False
            if curl > FIXED_SEPARATOR_THRESHOLD:
                if pred == 'translation_x_neg' and gate_margin <= TRANSLATION_MARGIN_GUARD_THRESHOLD:
                    ...
                    if curl > POSITIVE_GUARD_RESTORE_CURL_FLOOR:
                        row['positive_guard_overreach_restore_triggered'] = True
                        row['stage2_baseline_distance'] = db
                        row['stage2_rotation_distance'] = dr
                        pred = 'baseline' if db < dr else 'rotation_z_pos'
                        acts['positive_guard_overreach_restore_triggered'].append({'scale': s['scale'], 'seed': s['seed'], 'case_name': s['case_name'], 'label': s['label'], 'profile': s['profile'], 'gate_margin_translation_minus_nontranslation': gate_margin, 'bidirectional_veto_feature': curl, 'restored_prediction': pred})
                else:
...
def main() -> None:
    ap = argparse.ArgumentParser(description='Raw validation of the phase-2 positive guard overreach restore candidate on N288 harder target seeds 17-24.')
    ap.add_argument('--seen-root', required=True)
    ...
    payload = {
        'protocol': 'stage1_global_overview_farther_scale_n288_harder_phase2_positive_guard_overreach_raw_validation',
        'source_protocols': {
            'candidate_definition_audit': 'stage1_global_overview_farther_scale_n288_harder_phase2_positive_guard_overreach_candidate_definition_audit',
            'phase2_seedexp_source': 'stage1_global_overview_farther_scale_n288_harder_target_seedexp_phase2_fixed_v53_stack_candidate',
        },
        'seen_root': args.seen_root,
        'target_root': args.target_root,
        'selection_rule': 'carry forward the fixed v55 stack and add the phase-2 positive guard overreach restore candidate exactly as defined in v56; no threshold was changed after candidate definition',
        ...
        'fixed_stack': {
            ...
            'positive_guard_overreach_restore': {'curl_floor': POSITIVE_GUARD_RESTORE_CURL_FLOOR},
        },
        'seen_eval': seen_eval,
        'target_eval': target_eval,
        'seen_trigger_counts': {k: len(v) for k, v in seen_acts.items()},
        'target_trigger_counts': {k: len(v) for k, v in target_acts.items()},
        'seen_positive_guard_overreach_restore_triggered': seen_acts['positive_guard_overreach_restore_triggered'],
        'target_positive_guard_overreach_restore_triggered': target_acts['positive_guard_overreach_restore_triggered'],
        ...
        'verdict': 'raw_validation',
        'interpretation': [
            'if the target panel clears without seen pollution, ATLAS-R7 upgrades from candidate-defined to recoverable controlled family',
            'if residuals remain, the candidate definition gap was insufficient and the family remains unresolved',
        ],
    }

    json_path = outdir / 'stage1_global_overview_farther_scale_n288_harder_phase2_positive_guard_overreach_raw_validation_analysis.json'
    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    errs = payload['target_misclassifications']
    lines = [
        '# Stage-1 farther-scale N288 harder phase-2 positive guard overreach raw validation',
        '',
        'This protocol performs a raw-panel validation of the phase-2 positive-guard overreach restore candidate defined in v56. The fixed carried-forward stack is unchanged except for adding the candidate exactly as defined there.',
        '',
        '## Fixed carried-forward stack + candidate',
        ...
        f'- positive guard overreach restore: restore separator when the positive translation-margin guard would trigger but `{BIDIRECTIONAL_VETO_KEY} > {POSITIVE_GUARD_RESTORE_CURL_FLOOR:.6f}`',
        '',
        '## Results',
        ...
        f'- positive guard overreach restore: `{len(target_acts["positive_guard_overreach_restore_triggered"])} `',
        '',
    ...
    lines += [
        '',
        '## Interpretation',
        '- this result is only about ATLAS-R7 raw validation on the phase-2 target-seed panel',
        '- ATLAS-R8 remains unresolved and should not be silently folded into the same study',
    ]
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FARTHER_SCALE_N288_HARDER_PHASE2_POSITIVE_GUARD_OVERREACH_RAW_VALIDATION_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
