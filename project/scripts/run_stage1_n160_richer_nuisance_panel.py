...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run a richer nuisance panel at N160 for stage-1 readout validation.')
    p.add_argument('--outdir', type=str, default='outputs/stage1_n160_richer_nuisance_panel_raw')
...
def build_cases() -> dict[str, dict]:
    return {
        ...
        'translation_x_pos_mid_balanced': {
            'disable_gravity': True,
            'vestibular_motion': 'translation',
            'vestibular_linear_axis': 'x',
            'vestibular_linear_sign': 1.0,
            'vestibular_linear_accel': 300.0,
            'vestibular_onset_fraction': 0.44,
            'vestibular_duration_fraction': 0.46,
        },
        'translation_x_pos_late_soft': {
            'disable_gravity': True,
            'vestibular_motion': 'translation',
            'vestibular_linear_axis': 'x',
            'vestibular_linear_sign': 1.0,
            'vestibular_linear_accel': 240.0,
            'vestibular_onset_fraction': 0.58,
            'vestibular_duration_fraction': 0.50,
        },
        ...
        'translation_x_neg_mid_balanced': {
            'disable_gravity': True,
            'vestibular_motion': 'translation',
            'vestibular_linear_axis': 'x',
            'vestibular_linear_sign': -1.0,
        ...
        'rotation_z_pos_early_sharp': {
            ...
            'vestibular_angular_accel': 1800.0,
            'vestibular_onset_fraction': 0.25,
            'vestibular_duration_fraction': 0.32,
        },
        'rotation_z_pos_mid_balanced': {
            'disable_gravity': True,
            'vestibular_motion': 'rotation',
            'vestibular_rotation_axis': 'z',
            'vestibular_rotation_sign': 1.0,
            'vestibular_angular_accel': 1500.0,
            'vestibular_onset_fraction': 0.44,
            'vestibular_duration_fraction': 0.46,
        },
        'rotation_z_pos_late_soft': {
            'disable_gravity': True,
            'vestibular_motion': 'rotation',
            'vestibular_rotation_axis': 'z',
            'vestibular_rotation_sign': 1.0,
            'vestibular_angular_accel': 1200.0,
            'vestibular_onset_fraction': 0.58,
            'vestibular_duration_fraction': 0.50,
        },
    }
...
def main() -> None:
    ...
    manifest: dict[str, object] = {
        'protocol': 'stage1_n160_richer_nuisance_panel',
        'num_cells': 160,
        'seeds': args.seeds,
        't_end': args.t_end,
        'dt': args.dt,
    ...
    for seed in args.seeds:
        for case_name, overrides in cases.items():
            ...
            case_outdir = outdir / 'N160' / f'seed_{seed}' / case_name
            ...
            try:
                ...
                manifest['cases'][f'N160/seed_{seed}/{case_name}'] = {
                    'semantic_label': semantic_label(case_name),
                    'config': asdict(cfg),
                    'summary': summary,
                }
                print(f'[OK] N160 seed={seed} case={case_name}', flush=True)
                ...
                print(f'[ERR] N160 seed={seed} case={case_name}: {exc!r}', flush=True)
    (outdir / 'stage1_n160_richer_nuisance_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2))
    print(f'[OK] richer nuisance panel written to {outdir}')
