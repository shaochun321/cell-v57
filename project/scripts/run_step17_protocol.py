...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run the Step 17 unified-sphere + external readout protocol.")
    p.add_argument("--num-cells", type=int, default=180)
    p.add_argument("--t-end", type=float, default=0.18)
    p.add_argument("--dt", type=float, default=0.001)
    p.add_argument("--outdir", type=str, default="outputs/step17_protocol")
...
def _run_one(base_outdir: Path, name: str, **kwargs) -> dict:
    ...
    return {
        ...
        "readout_trace_samples": len(readout_trace),
    }
...
def main() -> None:
    ...
    report_path = outdir / "step17_protocol_report.json"
