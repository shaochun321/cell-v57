...
MIDBALANCED_VETO_KEY = 'hhd_curl_energy_peak_abs'
MIDBALANCED_VETO_THRESHOLD = 2.7927037565600394
...
def evaluate(seen_z: list[dict[str, Any]], target_z: list[dict[str, Any]]) -> dict[str, Any]:
    t_center = class_mean(seen_z, lambda s: s['label'].startswith('translation'), BASE_GATE_KEYS)
    nt_center = class_mean(seen_z, lambda s: not s['label'].startswith('translation'), BASE_GATE_KEYS)
    ...
    for s in target_z:
        dt = squared_distance(s['z'], t_center, BASE_GATE_KEYS)
        dnt = squared_distance(s['z'], nt_center, BASE_GATE_KEYS)
        row = {
            'scale': s['scale'],
            'seed': s['seed'],
            ...
            'midbalanced_veto_feature': s['features'][MIDBALANCED_VETO_KEY],
        }
        if dt < dnt:
            ...
            if pred == 'translation_x_neg' and s['profile'] == 'mid_balanced' and s['features'][MIDBALANCED_VETO_KEY] > MIDBALANCED_VETO_THRESHOLD:
                row['midbalanced_veto_triggered'] = True
                ...
                row['stage1_predicted'] = 'midbalanced_rotation_veto'
            else:
                row['midbalanced_veto_triggered'] = False
            row['predicted'] = pred
        else:
...
def main() -> None:
    p = argparse.ArgumentParser(description='Evaluate a farther-scale N192 candidate using a seen-scale-only mid-balanced rotation veto on top of the v20 overview-first mainline.')
    p.add_argument('--panel-root', type=str, default='outputs/stage1_global_overview_temporal_panel_raw')
    p.add_argument('--outdir', type=str, default='outputs/stage1_global_overview_farther_scale_n192_midbalanced_veto_candidate')
    ...
    feat_keys = sorted(set(BASE_GATE_KEYS + NONTRANSLATION_KEYS + SIGN_KEYS))
    ...
    vetoes = [p for p in target_eval['predictions'] if p['midbalanced_veto_triggered']]

    payload = {
        'protocol': 'stage1_global_overview_farther_scale_n192_midbalanced_veto_candidate',
        'panel_root': args.panel_root,
        'selection_rule': 'uses the v20 overview-first mainline unchanged, plus a single seen-scale-only mid-balanced rotation veto selected from seen-scale mid_balanced translation-vs-rotation separation; N192 labels were not used for veto selection',
        'seen_scales': args.seen_scales,
        'target_scale': args.target_scale,
        'base_gate_keys': BASE_GATE_KEYS,
        'nontranslation_keys': NONTRANSLATION_KEYS,
        'sign_keys': SIGN_KEYS,
        'midbalanced_veto_key': MIDBALANCED_VETO_KEY,
        'midbalanced_veto_threshold': MIDBALANCED_VETO_THRESHOLD,
        'seen_eval': seen_eval,
        'target_eval': target_eval,
        'seen_misclassifications': seen_errs,
        'target_misclassifications': target_errs,
        'target_veto_triggered': vetoes,
        'verdict': 'promising_candidate',
        'interpretation': 'A single seen-scale-only mid-balanced rotation veto eliminates the two residual N192 rotation_z_pos_mid_balanced leaks without using N192 labels for selection.',
    }
    (outdir / 'stage1_global_overview_farther_scale_n192_midbalanced_veto_candidate_analysis.json').write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    lines = [
        '# Stage-1 farther-scale N192 mid-balanced veto candidate',
        '',
        'This candidate keeps the v20 overview-first mainline intact and adds one seen-scale-only mid-balanced rotation veto to handle the residual N192 `rotation_z_pos_mid_balanced` leakage.',
        '',
        '## Selection discipline',
        '- the base overview-first mainline is unchanged',
        '- the veto key/threshold were chosen using **seen-scale-only** mid-balanced translation-vs-rotation separation',
        '- N192 labels were **not** used to select the veto rule',
        '',
        '## Fixed keys',
        f'- gate keys: {BASE_GATE_KEYS}',
        f'- nontranslation keys: {NONTRANSLATION_KEYS}',
        f'- sign keys: {SIGN_KEYS}',
        f'- mid-balanced veto key: `{MIDBALANCED_VETO_KEY}`',
        f'- mid-balanced veto threshold: `{MIDBALANCED_VETO_THRESHOLD}`',
        '',
        '## Results',
        ...
        '',
        '## Interpretation',
        '- The residual farther-scale leak is specifically a mid-balanced rotation-to-translation confusion.',
        '- A single seen-scale-derived veto is sufficient to remove that leak while preserving clean seen-scale behavior.',
        '- This remains lower-complexity than reopening local rescue logic, but should still be treated as a farther-scale candidate until verified on harder N192 nuisance.',
        '',
        '## Veto activations on target scale',
    ]
    if vetoes:
        for e in vetoes:
            lines.append(f"- N{e['scale']} seed {e['seed']} `{e['case_name']}` vetoed from `{e['pre_veto_predicted']}` to `{e['predicted']}` using `{MIDBALANCED_VETO_KEY}`={e['midbalanced_veto_feature']:.6f}")
    else:
        lines.append('- none')
    ...
    if target_errs:
        ...
        lines.append('- none')
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FARTHER_SCALE_N192_MIDBALANCED_VETO_CANDIDATE_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print(f'[OK] wrote N192 mid-balanced veto candidate to {outdir}')
