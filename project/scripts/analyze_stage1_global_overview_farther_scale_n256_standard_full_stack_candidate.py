...
def main() -> None:
    ...
    p.add_argument('--outdir', type=str, default='outputs/stage1_global_overview_farther_scale_n256_standard_full_stack_candidate')
    ...
    payload = {
        'protocol': 'stage1_global_overview_farther_scale_n256_standard_full_stack_candidate',
        ...
        'interpretation': 'This first beyond-N224 probe keeps the current full rule stack fixed and asks whether the overview-first geometry, the bidirectional separator, the negative sign-aware fallback, and the positive trace-basis bridge remain controlled at N256 under the standard richer-profile panel.'
    }
    (outdir / 'stage1_global_overview_farther_scale_n256_standard_full_stack_candidate_analysis.json').write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    lines = [
        '# Stage-1 farther-scale N256 standard full-stack candidate',
        '',
    ...
    lines += [
        ...
        '- if N256 standard is clean, the next honest move is N256 harder unseen-nuisance before broader promotion',
    ]
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FARTHER_SCALE_N256_STANDARD_FULL_STACK_CANDIDATE_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
