...
LATESOFT_SUPPLEMENT = 'temporal_agg_rotation_mid_to_late_peak_ratio'
GATE_KEYS = BASE_GATE_KEYS + [LATESOFT_SUPPLEMENT]
...
def main() -> None:
    p = argparse.ArgumentParser(description='Evaluate a fixed overview + HHD-lite + minimal late-soft timing supplement candidate.')
    p.add_argument('--panel-root', type=str, default='outputs/stage1_global_overview_temporal_panel_raw')
    p.add_argument('--outdir', type=str, default='outputs/stage1_global_overview_fixed_hhd_hybrid_latesoft_candidate')
    ...
    payload = {
        'protocol': 'stage1_global_overview_fixed_hhd_hybrid_latesoft_candidate',
        'panel_root': args.panel_root,
        'selection_rule': 'single-key late-soft timing supplement selected by seen-scale-only search from temporal overview features to repair the residual late_soft positive translation gate miss without using N160 labels',
        'seen_scales': args.seen_scales,
        'target_scale': args.target_scale,
        'base_gate_keys': BASE_GATE_KEYS,
        'late_soft_supplement': LATESOFT_SUPPLEMENT,
        'gate_keys': GATE_KEYS,
        ...
        'interpretation': 'A single temporal late-soft supplement repairs the only residual late_soft positive translation gate miss in the fixed+HHD hybrid candidate while preserving clean seen-scale compatibility.',
    }
    (outdir / 'stage1_global_overview_fixed_hhd_hybrid_latesoft_candidate_analysis.json').write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    lines = [
        '# Stage-1 fixed overview + HHD-lite + minimal late-soft timing supplement',
        '',
        'This candidate adds one seen-scale-selected temporal timing ratio to the fixed overview + HHD-lite hybrid to repair the residual `translation_x_pos_late_soft` miss.',
        '',
        '## Selection discipline',
        '- supplement selected by **seen-scale-only** search',
        '- target-scale labels were **not** used to choose the extra key',
        '',
        '## Fixed keys',
        f'- base gate keys: {BASE_GATE_KEYS}',
        f'- late-soft supplement: `{LATESOFT_SUPPLEMENT}`',
        f'- full gate keys: {GATE_KEYS}',
        ...
        '',
        '## Interpretation',
        '- A single timing ratio is sufficient to repair the residual late-soft miss in both seen scales and N160.',
        '- This keeps the overview-first branch low-complexity and avoids returning to local rescue logic.',
        '- The added key is timing-aware rather than probe-informed.',
        '',
        '## Remaining errors',
    ]
    if target_errs:
    ...
    if seen_errs:
        ...
        lines.append('- none')
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FIXED_HHD_HYBRID_LATESOFT_CANDIDATE_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print(f'[OK] wrote late-soft hybrid candidate to {outdir}')
