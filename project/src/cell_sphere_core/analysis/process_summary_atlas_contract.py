...
def validate_process_summary_atlas_analysis(payload: dict[str, Any]) -> dict[str, Any]:
    failures: list[str] = []
    ...
    for case_name in ("translation_x_pos", "rotation_z_pos"):
        phase_counts = cases[case_name].get("phase_coverage", {})
    ...
    tx_pos = float(cases["translation_x_pos"]["active_signature"].get("mean_polarity_projection", 0.0))
    tx_neg = float(cases["translation_x_neg"]["active_signature"].get("mean_polarity_projection", 0.0))
    _require(tx_pos * tx_neg < 0.0, f"translation process atlas polarity must flip sign, got {tx_pos} and {tx_neg}", failures)
    _require(abs(tx_pos - tx_neg) > 0.01, f"translation process atlas polarity separation too small: {abs(tx_pos - tx_neg)}", failures)

    rz_pos = float(cases["rotation_z_pos"]["active_signature"].get("mean_circulation_projection", 0.0))
    rz_neg = float(cases["rotation_z_neg"]["active_signature"].get("mean_circulation_projection", 0.0))
    _require(rz_pos * rz_neg < 0.0, f"rotation process atlas circulation must flip sign, got {rz_pos} and {rz_neg}", failures)
    _require(abs(rz_pos - rz_neg) > 0.01, f"rotation process atlas circulation separation too small: {abs(rz_pos - rz_neg)}", failures)
