...
def build_translation_x_pos_x_family_carrier_margin_strengthening_repair_audit(
    *,
    ...
    evidence = {
        'seed8_translation_x_pos_gap_reduced_vs_round38': current_gap < previous_gap,
        'seed8_translation_x_pos_active_amplitude_improved_vs_round38': txp8['active_mean_polarity_projection'] > float(previous_round.get('seed8', {}).get('translation_x_pos', {}).get('active_mean_polarity_projection', 0.0)),
        ...
        'seed8_translation_x_pos_margin_weighted_carrier_exceeds_current_mean': abs(txp8['active_margin_weighted_carrier_polarity_projection']) >= abs(txp8['active_mean_polarity_projection']),
        'seed8_translation_x_pos_margin_weighted_carrier_exceeds_round38_mean': abs(txp8['active_margin_weighted_carrier_polarity_projection']) >= abs(float(previous_round.get('seed8', {}).get('translation_x_pos', {}).get('active_mean_polarity_projection', 0.0))),
    }
    ...
    return {
        'suite': 'translation_x_pos_x_family_carrier_margin_strengthening_repair_audit_r1',
        ...
        'seed8': seed8,
        'evidence': evidence,
        'inferred_outcome': 'translation_x_pos_x_family_carrier_margin_strengthening_repair_success' if contracts_passed else 'undetermined',
        'residual_issue': residual_issue,
        'interpretation': {
            'primary_effect': 'strengthen the residual seed8 translation_x_pos x-family carrier margin by trusting positive-margin translation carriers more than weak static x pairs when the active x summary is still shallow',
            'guardrail': 'preserve x-axis identity, polarity sign, strongest-pair translation mode, and the rotation_z_pos z-axis guardrail',
            'next_branch': 'optional only if future rounds still need more seed8 x_pos carrier lift without broadening the repair scope beyond active x-family margin trust',
        },
    }
