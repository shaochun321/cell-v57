...
def build_translation_x_pos_seed8_residual_x_carrier_amplitude_restoration_repair_audit(
    *,
    ...
    seed7_reference_active_x = _phase_x_summary(seed7_reference_summary)
    ...
    baseline_shell2_second = _second_active_shell2_row(seed8_baseline_rows)
    repaired_shell2_second = _second_active_shell2_row(seed8_repaired_rows)

    baseline_gap = abs(float(seed7_reference_active_x.get('mean_polarity_projection', 0.0))) - abs(float(seed8_baseline_active_x.get('mean_polarity_projection', 0.0)))
    repaired_gap = abs(float(seed7_reference_active_x.get('mean_polarity_projection', 0.0))) - abs(float(seed8_repaired_active_x.get('mean_polarity_projection', 0.0)))

    evidence = {
        'repaired_seed8_translation_x_pos_active_mean_exceeds_round50': abs(float(seed8_repaired_active_x.get('mean_polarity_projection', 0.0))) > abs(float(seed8_baseline_active_x.get('mean_polarity_projection', 0.0))),
        'repaired_seed8_gap_to_seed7_reduced': repaired_gap < baseline_gap,
        ...
        'repaired_seed8_active_translation_carrier_total_preserved': int(seed8_repaired_counts['total']) == int(seed8_baseline_counts['total']),
        'repaired_seed8_shell_distribution_preserved': seed8_repaired_counts['per_shell'] == seed8_baseline_counts['per_shell'],
        'repaired_seed8_second_active_shell2_translation_like_preserved': bool(repaired_shell2_second) and repaired_shell2_second.get('dominant_mode') == 'translation_like',
        'repaired_seed8_second_active_shell2_positive_sign_preserved': bool(repaired_shell2_second) and float(repaired_shell2_second.get('direction_sign', 0.0)) > 0.0,
        'baseline_seed8_second_active_shell2_translation_like': bool(baseline_shell2_second) and baseline_shell2_second.get('dominant_mode') == 'translation_like',
    ...
    return {
        'suite': 'translation_x_pos_seed8_residual_x_carrier_amplitude_restoration_repair_audit_r1',
        ...
        'seed7_reference': {'phase_active_x_summary': seed7_reference_active_x},
        'seed8_round50_baseline': {
        ...
        'gaps': {
            'baseline_gap_to_seed7': baseline_gap,
            'repaired_gap_to_seed7': repaired_gap,
        },
        'evidence': evidence,
        'inferred_outcome': 'translation_x_pos_seed8_residual_x_carrier_amplitude_restoration_repair_success' if contracts_passed else 'undetermined',
