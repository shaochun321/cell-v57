...
def main() -> None:
    p = argparse.ArgumentParser(description='Evaluate the fixed v40 full rule stack on the first beyond-N224 harder unseen-nuisance panel.')
    p.add_argument('--panel-root', type=str, default='/mnt/data/v41_n256_harder_raw')
    p.add_argument('--outdir', type=str, default='outputs/stage1_global_overview_farther_scale_n256_harder_nuisance_full_stack_candidate')
        ...
        for s in dataset:
            ...
            if dt < dnt:
                ...
                neg_fallback = (
                    ...
                    neg_over_pos <= FIXED_NEG_OVER_POS_RATIO_THRESHOLD and gate_margin <= FIXED_GATE_MARGIN_BRIDGE_THRESHOLD
                )
    ...
    payload = {
        'protocol': 'stage1_global_overview_farther_scale_n256_harder_nuisance_full_stack_candidate',
        'panel_root': args.panel_root,
        'selection_rule': 'carry forward the v40 full rule stack unchanged into the first beyond-N224 harder unseen-nuisance panel: keep the overview-first mainline unchanged, keep the fixed bidirectional rotation separator unchanged, keep the negative sign-aware fallback unchanged, and keep the positive trace-basis bridge unchanged; no threshold reselection and no target-scale tuning',
        ...
        'verdict': 'beyond_n224_harder_full_stack_probe',
        'interpretation': 'This first beyond-N224 harder probe keeps the current full rule stack fixed and asks whether the overview-first geometry, the bidirectional separator, the negative sign-aware fallback, and the positive trace-basis bridge remain controlled at N256 under the harder unseen-nuisance panel.'
    }
    (outdir / 'stage1_global_overview_farther_scale_n256_harder_nuisance_full_stack_candidate_analysis.json').write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    lines = [
        '# Stage-1 farther-scale N256 harder unseen-nuisance full-stack candidate',
        '',
        'This probe carries forward the **current fixed rule stack** into the first beyond-N224 harder unseen-nuisance panel.',
        '',
        '## Fixed stack',
    ...
    lines += [
        '',
        '## Interpretation',
        '- this is the first beyond-N224 harder unseen-nuisance pressure test of the full current rule stack',
        ...
        '- if N256 harder is also clean, the next honest move is seed expansion or farther unseen scale before broader promotion',
    ]
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FARTHER_SCALE_N256_HARDER_NUISANCE_FULL_STACK_CANDIDATE_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
