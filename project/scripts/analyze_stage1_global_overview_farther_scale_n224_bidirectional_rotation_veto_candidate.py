...
def main() -> None:
    p = argparse.ArgumentParser(description='Evaluate the fixed v26 sign-agnostic direct rotation-veto candidate on a farther unseen scale beyond N192.')
    p.add_argument('--panel-root', type=str, default='tmp_v28_farther_scale_n224_standard_panel_raw')
    p.add_argument('--outdir', type=str, default='outputs/stage1_global_overview_farther_scale_n224_bidirectional_rotation_veto_candidate')
    ...
    payload = {
        'protocol': 'stage1_global_overview_farther_scale_n224_bidirectional_rotation_veto_candidate',
        'panel_root': args.panel_root,
        'selection_rule': 'keep the v26 sign-agnostic direct rotation-veto threshold fixed and evaluate the first farther unseen scale beyond N192 on the original standard richer-profile panel; no threshold reselection and no target-scale tuning',
        ...
        'verdict': 'farther_than_n192_standard_probe',
        'interpretation': 'This first farther-than-N192 probe keeps the v26 rule fixed and tests whether the overview-first geometry remains structured at N224 under the original standard richer-profile panel.',
    }
    (outdir / 'stage1_global_overview_farther_scale_n224_bidirectional_rotation_veto_candidate_analysis.json').write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    lines = [
        '# Stage-1 farther-scale N224 bidirectional rotation-veto candidate',
        '',
        'This audit keeps the **v26 sign-agnostic direct rotation veto** fixed and applies it to the first farther unseen scale beyond N192 under the original standard richer-profile panel.',
        '',
        ...
        '- target evaluation uses `N224` with seeds `7,8` on the standard richer-profile panel',
        '',
        '## Fixed keys',
    ...
    lines += [
        '',
        '## Interpretation',
        '- This is the first concrete beyond-N192 probe under a fixed farther-scale rule.',
        '- If the audit stays clean, the current route remains overview-first with the v26 farther-scale safeguard carried forward unchanged.',
        '- If the audit fails, that is evidence about the limit of the current farther-scale separator, not automatic proof that the physical core is dead.',
        '',
        '## Next task',
        '- Keep the overview-first mainline and the v26 farther-scale safeguard fixed.',
        '- If N224 standard passes cleanly, the next authoritative task is N224 harder unseen-nuisance validation before claiming a broader farther-scale regime.',
        '- If N224 standard fails, diagnose whether the residual is a narrow structured leak or a broader collapse before changing route.',
    ]
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FARTHER_SCALE_N224_BIDIRECTIONAL_ROTATION_VETO_CANDIDATE_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
