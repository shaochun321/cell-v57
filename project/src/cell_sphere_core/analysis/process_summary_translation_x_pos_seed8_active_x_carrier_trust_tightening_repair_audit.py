...
def build_translation_x_pos_seed8_active_x_carrier_trust_tightening_repair_audit(
    *,
    ...
    evidence = {
        "seed8_translation_x_pos_gap_reduced_vs_round42": current_gap < previous_gap,
        "seed8_translation_x_pos_active_amplitude_improved_vs_round42": txp8["active_mean_polarity_projection"] > float(previous_round.get("seed8", {}).get("translation_x_pos", {}).get("active_mean_polarity_projection", 0.0)),
        ...
        "seed8_translation_x_pos_weighted_floor_exceeds_round42_mean": abs(txp8["active_carrier_floor_weighted_polarity_projection"]) >= abs(float(previous_round.get("seed8", {}).get("translation_x_pos", {}).get("active_mean_polarity_projection", 0.0))),
        ...
        "seed8_translation_x_pos_raw_to_weighted_dilution_still_large": abs(txp8["active_raw_mean_polarity_projection"]) < 0.60 * max(abs(txp8["active_carrier_floor_weighted_polarity_projection"]), 1e-12),
    }
    ...
    return {
        "suite": "translation_x_pos_seed8_active_x_carrier_trust_tightening_repair_audit_r1",
        ...
        "seed8": seed8,
        "evidence": evidence,
        "inferred_outcome": "translation_x_pos_seed8_active_x_carrier_trust_tightening_repair_success" if contracts_passed else "undetermined",
        "residual_issue": residual_issue,
        "interpretation": {
            "primary_effect": "raise the repaired seed8 translation_x_pos mean slightly further by tightening trust around the same-sign floor-cleared active x carriers only when strongest-pair translation mode is already restored and the raw mean is still heavily diluted below the carrier-floor weighted signal",
            ...
            "next_branch": "optional only if future rounds still need more seed8 x_pos lift after trust tightening without broadening beyond active x-family summary corrections",
        },
    }
