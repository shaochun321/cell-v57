...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run Phase R.3 expanded closure audit for rotation stability.')
    ...
    p.add_argument('--rotation-alphas', type=str, default='380,420,460,500,540,580')
    p.add_argument('--swirl-gains', type=str, default='0.90,1.00,1.10,1.20,1.30,1.40')
    p.add_argument('--outdir', type=str, default='outputs/phase_r3_sample')
    return p.parse_args()
...
def main() -> None:
    ...
    central_gain = gains[len(gains)//2] if gains else 1.1
    report['translation_guard'] = _run_one(
        ...
        axial_base=max(0.68, 0.90 - 0.14 * max(0.0, central_gain - 1.0)),
    ...
    report_path = outdir / 'phase_r3_protocol_report.json'
    ...
    print(f'Phase R.3 审计: {audit_path}')
