...
def feature_ranges(rows: list[dict[str, Any]]) -> dict[str, dict[str, float]]:
    ...
    for row in rows:
        grouped.setdefault(row['label'], []).append(row['features'][BIDIRECTIONAL_VETO_KEY])
    return {
        label: {'min': min(vals), 'max': max(vals), 'mean': fmean(vals)}
        for label, vals in sorted(grouped.items())
    }


def main() -> None:
    p = argparse.ArgumentParser(description='Evaluate the fixed v26/v28 sign-agnostic direct rotation veto on an expanded N224 standard richer-profile seed panel.')
    p.add_argument('--panel-root', type=str, default='tmp_v29_farther_scale_n224_standard_seedexp_raw')
    p.add_argument('--outdir', type=str, default='outputs/stage1_global_overview_farther_scale_n224_standard_seedexp_bidirectional_candidate')
    ...
    payload = {
        'protocol': 'stage1_global_overview_farther_scale_n224_standard_seedexp_bidirectional_candidate',
        'panel_root': args.panel_root,
        'selection_rule': 'keep the v26/v28 sign-agnostic direct rotation-veto threshold fixed and broaden the N224 standard richer-profile target seed set; no threshold reselection and no target-scale tuning',
        ...
        'seen_feature_ranges': feature_ranges(seen),
        ...
        'verdict': 'n224_standard_seedexp_probe',
        'interpretation': 'This audit keeps the v26/v28 farther-scale rule fixed and tests whether N224 standard richer-profile behavior remains coherent under a broader target-seed set.',
    }
    (outdir / 'stage1_global_overview_farther_scale_n224_standard_seedexp_bidirectional_candidate_analysis.json').write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    lines = [
        '# Stage-1 farther-scale N224 standard seed-expansion bidirectional rotation-veto candidate',
        '',
        'This audit keeps the **v26/v28 sign-agnostic direct rotation veto** fixed and broadens the target seed set for the standard richer-profile N224 panel.',
        '',
        ...
        '- target evaluation uses `N224` with seeds `7,8,9,10,11,12` on the standard richer-profile panel',
        '',
        '## Fixed keys',
        ...
        f'- fixed threshold carried from v26/v28: `{FIXED_THRESHOLD}`',
        '',
        '## Seen-scale feature ranges supporting the rule',
    ]
    for label, stats in feature_ranges(seen).items():
    ...
    lines += [
        '',
        '## Interpretation',
        '- The fixed farther-scale separator does not generically collapse at N224 under seed expansion.',
        '- The current residual is narrow: one late-soft negative-translation case crosses the stage-1 translation/nontranslation boundary and lands in baseline.',
        '- This is evidence of a structured farther-scale residual, not automatic proof that the overview-first route or the physical core should be abandoned.',
        '',
        '## Next task',
        '- Keep the overview-first mainline and the v26/v28 farther-scale safeguard fixed.',
        '- Diagnose the remaining N224 `translation_x_neg_late_soft -> baseline` residual before inventing another farther-scale rule family.',
        '- Only after that, decide whether harder N224 seed expansion or a still farther unseen scale is the better next pressure test.',
    ]
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FARTHER_SCALE_N224_STANDARD_SEEDEXP_BIDIRECTIONAL_CANDIDATE_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
