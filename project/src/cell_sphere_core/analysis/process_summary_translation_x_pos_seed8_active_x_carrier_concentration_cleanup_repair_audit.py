...
def build_translation_x_pos_seed8_active_x_carrier_concentration_cleanup_repair_audit(
    *,
    ...
    evidence = {
        "seed8_translation_x_pos_gap_reduced_vs_round40": current_gap < previous_gap,
        "seed8_translation_x_pos_active_amplitude_improved_vs_round40": txp8["active_mean_polarity_projection"] > float(previous_round.get("seed8", {}).get("translation_x_pos", {}).get("active_mean_polarity_projection", 0.0)),
        ...
        "seed8_translation_x_pos_weighted_floor_exceeds_round40_mean": abs(txp8["active_carrier_floor_weighted_polarity_projection"]) >= abs(float(previous_round.get("seed8", {}).get("translation_x_pos", {}).get("active_mean_polarity_projection", 0.0))),
    ...
    return {
        "suite": "translation_x_pos_seed8_active_x_carrier_concentration_cleanup_repair_audit_r1",
        ...
        "seed8": seed8,
        "evidence": evidence,
        "inferred_outcome": "translation_x_pos_seed8_active_x_carrier_concentration_cleanup_repair_success" if contracts_passed else "undetermined",
        "residual_issue": residual_issue,
        "interpretation": {
            "primary_effect": "raise the repaired seed8 translation_x_pos mean by trusting the concentration-weighted subset of same-sign active x carriers that already clear the carrier floor, instead of letting weak static x rows keep diluting the final active summary",
            ...
            "next_branch": "optional only if future rounds still need more seed8 x_pos lift after concentration cleanup without broadening beyond active x-family summary corrections",
        },
    }
