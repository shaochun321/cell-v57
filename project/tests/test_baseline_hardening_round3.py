...
def test_run_and_analyze_baseline_hardening_round3(tmp_path: Path) -> None:
    env = dict(os.environ)
    env.setdefault('MPLCONFIGDIR', str((tmp_path / '.mplconfig').resolve()))
    env['PYTHONPATH'] = str(Path.cwd() / 'src') + os.pathsep + env.get('PYTHONPATH', '')
    outdir = tmp_path / 'hardening3'

    subprocess.run([
        sys.executable, 'scripts/run_baseline_hardening_round3.py',
    ...
    subprocess.run([
        sys.executable, 'scripts/analyze_baseline_hardening_round3.py',
        '--input', str(report_path), '--output', str(outdir / 'analysis')
    ], check=True, env=env)

    analysis_path = outdir / 'analysis' / 'baseline_hardening_round3_analysis.json'
    png_path = outdir / 'analysis' / 'baseline_hardening_round3_overview.png'
    assert analysis_path.exists()
    assert png_path.exists()

    payload = json.loads(analysis_path.read_text(encoding='utf-8'))
    assert_margin(payload['static']['quietness_score'], eps=0.688, label='round3 static quietness')
    assert payload['static']['tail_kinetic_mean'] < 0.455, payload['static']['tail_kinetic_mean']
    assert_margin(payload['static']['shape_score'], eps=0.70, label='round3 static shape score')
    ...
    for track in TRACKS:
        ...
        assert_margin(t['x_axis_balance_abs_mean'], eps=0.007, label=f'{track} translation amplitude')
        assert_margin(t['mean_translation_margin'], eps=0.085, label=f'{track} translation margin')
