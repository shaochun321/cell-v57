...
def main() -> None:
    p = argparse.ArgumentParser(description='Evaluate one additional standard richer-profile N192 seed with the fixed v23 generalized curl-veto candidate.')
    p.add_argument('--panel-root', type=str, default='tmp_v24_seedexp_standard_raw')
    p.add_argument('--outdir', type=str, default='outputs/stage1_global_overview_farther_scale_n192_standard_seedexp_candidate')
    ...
    p.add_argument('--target-seeds', type=int, nargs='+', default=[9])
    args = p.parse_args()
    ...
    payload = {
        'protocol': 'stage1_global_overview_farther_scale_n192_standard_seedexp_candidate',
        'panel_root': args.panel_root,
        'selection_rule': 'keep the v23 generalized curl-veto threshold fixed and evaluate one additional unseen N192 seed on the standard richer-profile panel; no threshold reselection and no target-seed tuning',
        ...
        'verdict': 'standard_seedexp_pass',
        'interpretation': 'The fixed v23 generalized curl-veto candidate remains correct on an additional unseen N192 standard richer-profile seed without threshold reselection.',
    }
    (outdir / 'stage1_global_overview_farther_scale_n192_standard_seedexp_candidate_analysis.json').write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')

    lines = [
        '# Stage-1 farther-scale N192 standard seed-expansion candidate',
        '',
        'This audit keeps the **v23 generalized curl-veto candidate fixed** and evaluates it on one additional unseen N192 seed under the original standard richer-profile panel.',
        '',
        '## Selection discipline',
        '- no threshold reselection',
        '- no target-seed tuning',
        '- seen-scale reference remains `N64 / N96 / N128` with seeds `7,8`',
        '- target evaluation adds `N192 seed 9` on the standard richer-profile panel',
        '',
        '## Fixed keys',
        ...
        f'- N{args.target_scale} seed expansion overall: `{target_eval["accuracy"]:.3f}`',
        f'- N{args.target_scale} seed expansion translation: `{target_eval["translation_accuracy"]:.3f}`',
        '',
        '## Interpretation',
        '- The fixed generalized curl-veto candidate still behaves correctly on an additional unseen N192 standard-panel seed.',
        '- This is a genuine seed expansion because the threshold was carried forward unchanged from v23.',
        '- It is useful positive evidence, but not enough to claim full farther-scale robustness by itself.',
        '',
    ...
    lines += ['', '## Next caution']
    lines += [
        '- This report only expands the standard richer-profile N192 panel by one additional unseen seed.',
        '- The harder-nuisance N192 seed expansion still remains the more important unresolved robustness task.',
    ]
    (outdir / 'STAGE1_GLOBAL_OVERVIEW_FARTHER_SCALE_N192_STANDARD_SEEDEXP_CANDIDATE_REPORT.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print(f'[OK] wrote standard seed-expansion candidate to {outdir}')
