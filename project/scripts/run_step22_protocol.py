...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run the Step 22 interface-spectrum protocol.")
    p.add_argument("--num-cells", type=int, default=180)
    p.add_argument("--t-end", type=float, default=0.18)
    p.add_argument("--dt", type=float, default=0.001)
    p.add_argument("--outdir", type=str, default="outputs/step22_protocol")
...
def _extract_spectrum_report(summary: dict) -> dict:
    diag = summary.get("interface_spectrum_diagnostics", {})
    tracks = diag.get("tracks", {})
    out: dict[str, dict] = {}
    for name, payload in tracks.items():
        out[name] = {
            "family_cluster_means": payload.get("family_cluster_means", {}),
            "active_family_cluster_means": payload.get("active_family_cluster_means", {}),
            "mean_bandwidth_proxy": payload.get("mean_bandwidth_proxy", 0.0),
            "active_mean_bandwidth_proxy": payload.get("active_mean_bandwidth_proxy", 0.0),
            "mean_signed_polarity": payload.get("mean_signed_polarity", 0.0),
            "mean_signed_circulation": payload.get("mean_signed_circulation", 0.0),
            "active_mean_signed_polarity": payload.get("active_mean_signed_polarity", 0.0),
            "active_mean_signed_circulation": payload.get("active_mean_signed_circulation", 0.0),
            "axis_balance": payload.get("axis_balance", {}),
            "active_axis_balance": payload.get("active_axis_balance", {}),
            "mean_transfer_variability": payload.get("mean_transfer_variability", 0.0),
            "active_mean_transfer_variability": payload.get("active_mean_transfer_variability", 0.0),
            "mean_axial_inter_shell_delta": payload.get("mean_axial_inter_shell_delta", 0.0),
            "mean_swirl_inter_shell_delta": payload.get("mean_swirl_inter_shell_delta", 0.0),
        }
    return out
...
def _run_one(base_outdir: Path, name: str, **kwargs) -> dict:
    ...
    return {
        ...
        "spectrum_report": _extract_spectrum_report(result.summary),
    }
